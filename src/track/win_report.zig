//! One-shot win report: correctness + PAR-2 + multishot + HWMCC + embed size.
...
fn fileSize(path: []const u8) ?u64 {
    ...
    // Seek end for size (no libc stat type games on zig 0.16).
...
pub const WinReport = struct {
    ...
    pub fn allRequired(self: *const WinReport) bool {
        // embed_win is soft if cadical missing; fair soft if slow
        return self.correctness_pass and self.par2_win and self.multishot_win and self.hwmcc_pass;
    }
};
...
pub fn printScoreboard(r: *const WinReport) void {
    ...
    std.debug.print("embed:        {s}\n", .{if (r.embed_win) "WIN" else "LOSE_OR_NA"});
