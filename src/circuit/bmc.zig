...
fn blastFrame(cnf: *Cnf, nl: *const Netlist, frame: u32) !void {
    ...
    for (nl.gates.items) |g| {
        ...
        switch (g.kind) {
            ...
            .and_, .and_n => {
                for (g.inputs) |inp| {
                    const a = Lit.positive(frameVar(nn, frame, inp));
                    try cnf.addClause(&.{ y.not(), a });
                }
                var lits: std.ArrayList(Lit) = .empty;
                defer lits.deinit(cnf.allocator);
                for (g.inputs) |inp| {
                    try lits.append(cnf.allocator, Lit.negative(frameVar(nn, frame, inp)));
                }
            ...
            .or_, .or_n => {
                ...
                for (g.inputs) |inp| {
                    try lits.append(cnf.allocator, Lit.positive(frameVar(nn, frame, inp)));
                }
                try cnf.addClause(lits.items);
            },
            .xor => {
